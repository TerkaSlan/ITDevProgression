...
int main()
{
    ...
    int i,j=1, counter=0, tempHeight[15], height=0;
    for(i=0; i<15; i++) {tempHeight[i] = -1;}
    i=0;    k=0;
    ...
    while(counter < filesize){
        while(*buffer != '\n' && counter < filesize){
            ...
            if(i==1)    {tempHeight[j] = *buffer-'0'; j++;}//j-1 because i don't need to keep address[0] -1
            buffer++;
            counter++;
        }
        if(i==1)    height = makeNumber(&tempHeight[j-1]);
        if(height > 210)    {printf("\n%u ? Oh come on, what are you, Hulk or something?!", height); return -1;}

        j=1;
        i++;
        if(i==2){//i have all the info to make a new entry
            strcpy(studentDatabase[k].name, tempName);
            studentDatabase[k].height = height;
            k++;

            for(i=0; i<15; i++) {tempHeight[i] = -1;} // erase everything
    ...
    for(i=0; i<100; i++){// i have 3 house entries, there will be 100 on maturita
        if(studentDatabase[i].name[0]!='\0')    printf("[STUDENT %d] Name: %s, is %u cm tall\n", i+1, studentDatabase[i].name, studentDatabase[i].height);
        else{break;}
    }
    ...
    for(k=0; k<i-1; k++){
        j=k;
        while(j>0 && studentDatabase[j].height < studentDatabase[j-1].height){
            temp = studentDatabase[j].height;
            studentDatabase[j].height = studentDatabase[j-1].height;
            studentDatabase[j-1].height = temp;
    ...
    printf("\n\n5 tallest students:\n");
    for(j=i-2; j>i-5-2; j--){
        printf("[BASKETBALL PLAYER %d] is %s and has %u cm.\n", i-j-1, studentDatabase[j].name, studentDatabase[j].height);
    }
    return 0;
}
